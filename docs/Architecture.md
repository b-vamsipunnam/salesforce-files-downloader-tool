...
- **Robot Framework** coordinates suite initialization, per-batch API preflight, input validation, metadata mapping, downloads, validation, retry state, reporting, and teardown.
...
- **Capacity protection:** each batch estimates tool requests and preserves a configurable daily API reserve before creating artifacts.
