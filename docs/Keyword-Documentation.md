...
| `Find Completed Download File`       | Select a completed browser file.         | `${recent_files}`                                                                        | None                             | Sets test-scoped `${latest_filename}` and `${is_filename_proper}`; excludes temporary suffixes. |
